...
- **Blueprint** = grid parts + wires + logic nodes. Canonical string + FNV hash.
  Two equivalent builds always hash identically. Test mode snapshots the
...
3. Power network solves (15 Hz) → what is powered, voltage sag, fuse trips.
...
The client simulates locally and trusts its own inputs; the peer's inputs arrive
over the wire.
